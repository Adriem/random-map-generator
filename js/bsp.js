...
var split = function (sector, horizontalDirection, steps) {
	...
	if (horizontalDirection) {
		if(steps === 0 ||  (sector.h < SECTOR_MAX_SIZE && randomTest(BIG_ROOM_CHANCE)) ||
				sector.h < 2*SECTOR_MIN_SIZE || sector.h/sector.w < 2*RATIO_RESTR) return [sector];
		...
		if(steps === 0 || (sector.h < SECTOR_MAX_SIZE && randomTest(BIG_ROOM_CHANCE)) ||
				sector.w < 2*SECTOR_MIN_SIZE || sector.w/sector.h < 2*RATIO_RESTR) return [sector];
